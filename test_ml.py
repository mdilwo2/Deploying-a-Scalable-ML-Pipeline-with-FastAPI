...
def processed_data():
    X, y, encoder, lb = process_data(
        data, categorical_features=['workclass', 'education', 'marital-status', 'occupation', 'relationship', 'race', 'sex', 'native-country']
...
def test_process_data(processed_data):
    ...
    assert isinstance(X, np.ndarray), "Expected X to be a DataFrame"
    assert isinstance(y, np.ndarray), "Expected y to be a Series"
...
def test_compute_model_metrics_value(processed_data):
    ...
    assert f1 >= 0 and f1 <= 1, "Expected f1 to be between 0 and 1"
